...
const approveVoucherAPI = async (id) => {
  // Simulating API call
  await new Promise(resolve => setTimeout(resolve, 1000))
  return { success: true, message: `Voucher ${id} approved successfully` }
}
...
const initialVouchers = [
  {
    ...
    StateCode: "KA",
    VendorTaxClassification: "Regular",
    GSTIN: "29ABCDE1234F1Z5",
    AccountingDate: "2023-06-16",
    LineNarration: "Office Supplies",
    HoldAmount: 0,
    HoldReason: "",
    hasTDS: "N",
    Inward_No: "INW001",
    ...
    VoucherLink: "NV",
    VendName: "ABC Suppliers",
    VAddress1: "123 Main St, Bangalore",
    PanNo: "ABCDE1234F",
    BankName: "XYZ Bank",
...
export default function VoucherApproval() {
  ...
  useEffect(() => {
    const fetchVouchers = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/v1/user/vouchers');
    ...
    fetchVouchers(); // Call the function when the component mounts
  }, []);
  ...
  const approveVoucher = async (id) => {
    setIsApproving(true)
    try {
      const result = await approveVoucherAPI(id)
      if (result.success) {
